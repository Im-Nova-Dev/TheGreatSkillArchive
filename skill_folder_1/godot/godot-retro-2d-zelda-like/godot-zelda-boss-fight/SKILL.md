---
name: godot-zelda-boss-fight
description: Godot Zelda Boss Fight
---

# Godot Zelda Boss Fight

## Core Concepts
Pattern-based boss.
```gdscript
class_name Boss extends CharacterBody2D

var pattern_index := 0

func next_pattern() -> void:
    pattern_index = (pattern_index + 1) % patterns.size()
    execute_pattern(patterns[pattern_index])

func execute_pattern(p: Dictionary) -> void:
    match p.type:
        "charge": charge()
        "spawn": spawn_minions()
        "jump": jump_attack()
```

## Learning Path

1. **Foundation**: Study the core concepts.
2. **Implementation**: Build a small demo in Godot 4.
3. **Deep dive**: Polish and expand with more features.
4. **Production**: Make it a complete game.

## Common Pitfalls

- Scope creep: start tiny.
- Polishing before gameplay is locked.
- Forgetting pixel-snap camera.
- Over-engineering systems.

## Best Practices

- Keep one-click export ready.
- Profile 60 FPS target.
- Use Godot primitives (TileMap, AnimationPlayer).
- Ship one level, then iterate.

## Resources

- Godot 4 docs
- GDQuest, HeartBeast, 41b
- /r/godot
- itch.io devlogs
- Game jam communities
