---
name: godot-retro-2d-polish-screen-shake
description: Godot Retro 2D Polish Screen Shake
---

# Godot Retro 2D Polish Screen Shake

## Core Concepts
Subtle screen shake with decay.
```gdscript
extends Camera2D

var intensity := 0.0
var duration := 0.0

func shake(amount: float, time: float) -> void:
    intensity = amount
    duration = time

func _process(delta: float) -> void:
    if duration > 0:
        offset = Vector2(randf() - 0.5, randf() - 0.5) * intensity
        duration -= delta
    else:
        offset = Vector2.ZERO
```

## Common Pitfalls

- Overcomplicating scope before the core loop is confirmed fun.
- Polishing visuals before gameplay feels right.
- Missing one-click export workflow until submit day.
- Ignoring Godot’s built-in TileMap and Animation tools.

## Best Practices

- Scope to one screen first.
- Profile every build.
- Ship one polished level instead of three rough ones.
- Keep art in consistent palette and resolution.

## Resources

- Godot 4 class reference
- GDQuest retro/2D tutorials
- /r/godot
- Game jam communities (Ludum Dare, GMTK)
- itch.io devlogs
